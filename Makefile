...
PACKAGE := rptadv-ffmpeg-adapter
CRATE := rptadv_ffmpeg_adapter
...
DEBIAN_RUNTIME_DEB = $(DEBIAN_OUTPUT_DIR)/librptadv-ffmpeg-adapter1_$(DEBIAN_VERSION)_$(DEBIAN_ARCH).deb
DEBIAN_DEV_DEB = $(DEBIAN_OUTPUT_DIR)/librptadv-ffmpeg-adapter-dev_$(DEBIAN_VERSION)_$(DEBIAN_ARCH).deb
...
.PHONY: all quality lint static-analysis docs test install install-check dist \
	quality-image container-test clean FORCE
...
$(TARGET_LIBRARY): Cargo.toml Cargo.lock build.rs $(RUST_SOURCES) $(C_BRIDGE_SOURCES)
	CARGO_TARGET_DIR="$(CARGO_TARGET_DIR)" RUSTFLAGS="$(SONAME_RUSTFLAGS)" \
...
lint:
	$(CARGO_FMT) --check
	$(SHELLCHECK) $(QUALITY_LAUNCHER) tools/coverage-cc.sh
...
container-test: quality-image
	RPTADV_CONTAINER_PULL=0 sh $(QUALITY_LAUNCHER) $(QUALITY_IMAGE) $(MAKE) test install-check
...
install: all $(PC_FILE)
	install -d $(DESTDIR)$(LIBDIR) \
		$(DESTDIR)$(PREFIX)/include/rptadv_ffmpeg_adapter \
	...
	install -m 0644 $(HEADER) $(DESTDIR)$(PREFIX)/include/rptadv_ffmpeg_adapter/
...
install-check: all
	...
	$(READELF) -d build/stage/usr/lib/$(notdir $(LIBRARY_VERSIONED)) | grep -F 'libavfilter.so'
	$(READELF) -d build/stage/usr/lib/$(notdir $(LIBRARY_VERSIONED)) | grep -F 'libavutil.so'
	test -f build/stage/usr/include/rptadv_ffmpeg_adapter/$(notdir $(HEADER))
	test -f build/stage/usr/lib/pkgconfig/rptadv_ffmpeg_adapter.pc
	$(CC) -std=c11 -Wall -Wextra -Werror $(C_SMOKE_SOURCE) \
		$$(PKG_CONFIG_LIBDIR="$(CURDIR)/build/stage/usr/lib/pkgconfig" \
			...
			pkg-config --cflags --libs rptadv_ffmpeg_adapter) \
...
debian-package-check: dist
	...
	$(READELF) -d "$(DEBIAN_STAGE)/usr/lib/$(DEBIAN_MULTIARCH)/$(LIBRARY_BASENAME).so.$(SOVERSION)" | grep -F 'libavfilter.so'
	! $(READELF) -d "$(DEBIAN_STAGE)/usr/lib/$(DEBIAN_MULTIARCH)/$(LIBRARY_BASENAME).so.$(SOVERSION)" | grep -E 'res_usbradio.so|libportaudio.so|libasound.so|libhamlib.so'
...
dist: | build
	...
	tar --exclude=.git --exclude=.work --exclude=build --exclude=target \
		...
		--exclude=debian/librptadv-ffmpeg-adapter1 --exclude=debian/librptadv-ffmpeg-adapter-dev \
		...
		rptadv_ffmpeg_adapter.pc.in rust-toolchain.toml build.rs containers debian include src tests tools

distcheck: dist
	! tar -tzf build/$(PACKAGE)-$(PACKAGE_VERSION).tar.gz | \
		grep -E '/debian/(\.debhelper/|debhelper-build-stamp$$|files$$|tmp/|librptadv-ffmpeg-adapter(1|-dev)/|.*\.(substvars|debhelper\.log)$$)'
