...
# Build a deterministic local quality image from the maintained Debian 13 base.
...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
		ca-certificates cargo curl debhelper dpkg-dev libavfilter-dev libavutil-dev pkg-config rustc && \
	rm -rf /var/lib/apt/lists/*

# Pin the compiler used to build the public ABI rather than inheriting a
# changing distro Rust release. The architecture-specific checksum keeps the
# bootstrap deterministic on the two native Debian 13 targets.
...
RUN rustup toolchain install "${RUST_NIGHTLY}" --profile minimal --component llvm-tools-preview && \
